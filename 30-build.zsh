...
setup_build_parameters() {
  ...
  typeset -g -a c_flags=(-w -pipe -fsanitize=address)
  typeset -g -a cxx_flags=(-w -pipe -fsanitize=address)
  typeset -g -a ld_flags=(-fsanitize=address)
  ...
  case "${1}" in
    macos*)
      ...
      cmake_flags+=(
        ...
        -DCMAKE_XCODE_SCHEME_ADDRESS_SANITIZER=ON
        -DCMAKE_XCODE_SCHEME_ADDRESS_SANITIZER_USE_AFTER_RETURN=ON
      )
